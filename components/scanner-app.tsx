...
import { ScannerExamSetup } from '@/components/scanner-exam-setup';
import { CsvExportPanel } from '@/components/csv-export-panel';
import { decodeCompactResult, parseScannerExam, SCANNER_EXAM_KEY, type ScannerExam } from '@/lib/compact-qr';
...
type ScanMode = 'compact' | 'simple' | 'complete';
...
export function ScannerApp() {
  ...
  const [exam, setExam] = useState<ScannerExam>();
  const examRef = useRef<ScannerExam | undefined>(undefined);
  ...
  useEffect(() => {
    ...
    scanModeRef.current = savedMode === 'compact' || savedMode === 'complete' ? savedMode : 'simple';
    setScanMode(scanModeRef.current);
    const savedExam = localStorage.getItem(SCANNER_EXAM_KEY);
    if (savedExam) {
      void (async () => {
        try { const restored = await parseScannerExam(JSON.parse(savedExam)); setExam(restored); examRef.current = restored; }
        catch { setError('Reload your scanner setup to use one-QR results.'); }
      })();
    }
    void loadCameras();
  ...
  function chooseScanMode(mode: ScanMode) {
    ...
    setError('');
    setMessage(`${mode === 'compact' ? 'One QR' : mode === 'simple' ? 'Simple' : 'Full'} result mode selected`);
  }

  function loadExam(next: ScannerExam) {
    localStorage.setItem(SCANNER_EXAM_KEY, JSON.stringify(next));
    stopScanner();
    examRef.current = next; setExam(next);
    setPreview(undefined); setDuplicate(undefined);
    partsRef.current = {}; setParts({});
    chooseScanMode('compact');
  }
  ...
  async function processScannedValue(value: string) {
    ...
    try {
      ...
      if (/^\d{2}-/.test(value.trim())) {
        if (!examRef.current) throw new Error('Load the matching scanner setup first, then scan this QR again.');
        await showPreview(decodeCompactResult(value, examRef.current));
        return;
      }
  ...
  async function storePreview() {
    ...
    if (duplicate?.resultId === preview.resultId) {
      ...
      setTab('scan');
      setMessage('Already saved. Ready for the next student.');
...
function MoreView({ results, importRef, setMessage, refresh, setError }: any) {
  ...
  return <div>
    <div className="mb-7"><p className="section-eyebrow">Data tools</p><h1 className="section-title">Export & backup</h1></div>
    <CsvExportPanel results={results} />
    <div className="grid gap-5 md:grid-cols-3">
      <ToolCard icon={Download} title="Backup database" copy="Download every stored result." action="Backup database" onClick={backup} />
      <ToolCard icon={ArchiveRestore} title="Restore backup" copy="Merge a backup or replace the current local result database." action="Restore backup" onClick={() => importRef.current?.click()} />
      <ToolCard icon={Trash2} danger title="Clear all data" copy="Permanently remove every scanned result from this browser." action="Clear all results" disabled={!results.length} onClick={async () => { if (!confirm(`Delete all ${results.length} stored results? This cannot be undone.`)) return; await clearAllResults(); setMessage('All local results cleared'); refresh(); }} />
    </div>
    <p className="mt-5 text-sm text-muted-foreground">Results stay on this device. Keep your scanner setup file and a database backup before changing devices or clearing browser storage.</p>
  </div>;
}
