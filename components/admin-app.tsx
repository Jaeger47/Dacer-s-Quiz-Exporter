...
type Section = 'information' | 'questions' | 'students' | 'settings' | 'security' | 'preview' | 'export';
...
const sections: { id: Section; label: string; icon: typeof BookOpenText }[] = [
  ...
  { id: 'students', label: 'Students / One QR', icon: BookOpenText },
...
export function AdminApp() {
  ...
  async function exportStudentQuiz() {
    ...
    try {
      ...
      if (quiz.settings.resultDataMode === 'compact') localStorage.setItem(SCANNER_EXAM_KEY, JSON.stringify(await createScannerExam(quiz)));
      ...
      <div className="mx-auto grid max-w-[1600px] lg:grid-cols-[260px_minmax(0,1fr)]">
        ...
        <section className="min-w-0 px-4 py-6 sm:px-6 lg:px-10 lg:py-9 xl:px-14">
          <div className="mx-auto max-w-6xl">
            ...
            {section === 'questions' && <QuestionsSection quiz={quiz} updateQuestion={updateQuestion} addQuestion={addQuestion} duplicateQuestion={duplicateQuestion} deleteQuestion={deleteQuestion} moveQuestion={moveQuestion} changeType={changeType} onContinue={() => setSection('students')} />}
            {section === 'settings' && <SettingsSection quiz={quiz} updateSetting={updateSetting} onContinue={() => setSection('security')} />}
            {section === 'students' && <StudentRoster quiz={quiz} onChange={setQuiz} />}
